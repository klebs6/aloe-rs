...
//-------------------------------------------[.cpp/Aloe/modules/aloe_core/maths/aloe_MathsFunctions.h]
...
bool isPositiveAndBelow (Type1 valueToTest, Type2 upperLimit) 
{
...
template <typename Type>
bool isPositiveAndBelow (int valueToTest, Type upperLimit) 
{
...
*/
template <typename Type1, typename Type2>
bool isPositiveAndNotGreaterThan (Type1 valueToTest, Type2 upperLimit) 
{
    jassert (Type1() <= static_cast<Type1> (upperLimit)); // makes no sense to call this if the upper limit is itself below zero..
    return Type1() <= valueToTest && valueToTest <= static_cast<Type1> (upperLimit);
}
...
template <typename Type>
bool isPositiveAndNotGreaterThan (int valueToTest, Type upperLimit) 
{
    jassert (upperLimit >= 0); // makes no sense to call this if the upper limit is itself below zero..
    return static_cast<unsigned int> (valueToTest) <= static_cast<unsigned int> (upperLimit);
}
...
*/
template <typename Type>
bool isWithin (Type a, Type b, Type tolerance) 
{
...
*/
template <typename Type>
bool approximatelyEqual (Type a, Type b) 
{
...
constexpr int numElementsInArray (Type (&)[N])      { return N; }

// Some useful maths functions that aren't always present with all compilers and build settings.

/** Using aloe_hypot is easier than dealing with the different types of hypot function
    that are provided by the various platforms and compilers. */
template <typename Type>
Type aloe_hypot (Type a, Type b) 
{
   #if ALOE_MSVC
...
#ifndef DOXYGEN
template <>
inline float aloe_hypot (float a, float b) 
{
   #if ALOE_MSVC
    return _hypotf (a, b);
   #else
...
template <typename FloatType>
constexpr FloatType degreesToRadians (FloatType degrees)      { return degrees * (MathConstants<FloatType>::pi / FloatType (180)); }

/** Converts an angle in radians to degrees. */
template <typename FloatType>
constexpr FloatType radiansToDegrees (FloatType radians)      { return radians * (FloatType (180) / MathConstants<FloatType>::pi); }
...
bool aloe_isfinite (NumericType) 
{
    return true; // Integer types are always finite
}

template <>
inline bool aloe_isfinite (float value) 
{
   #if ALOE_WINDOWS && ! ALOE_MINGW
...
template <>
inline bool aloe_isfinite (double value) 
{
   #if ALOE_WINDOWS && ! ALOE_MINGW
    return _finite (value) != 0;
   #else
...
inline int roundToInt (int value) 
{
    return value;
}
...
*/
inline int roundToIntAccurate (double value) 
{
   #ifdef __INTEL_COMPILER
    #pragma float_control (pop)
   #endif
...
*/
template <typename IntegerType>
IntegerType negativeAwareModulo (IntegerType dividend, const IntegerType divisor) 
{
    jassert (divisor > 0);
    dividend %= divisor;
...
template <typename NumericType>
inline constexpr NumericType square (NumericType n) 
{
    return n * n;
}
...
*/
void writeLittleEndianBitsInBuffer (void* targetBuffer, uint32 startBit, uint32 numBits, uint32 value) ;
...
*/
uint32 readLittleEndianBitsInBuffer (const void* sourceBuffer, uint32 startBit, uint32 numBits) ;


#if ALOE_INTEL || defined (DOXYGEN)
 /** This macro can be applied to a float variable to check whether it contains a denormalised
     value, and to normalise it if necessary.
     On CPUs that aren't vulnerable to denormalisation problems, this will have no effect.
 */
 #define ALOE_UNDENORMALISE(x)   { (x) += 0.1f; (x) -= 0.1f; }
#else
 #define ALOE_UNDENORMALISE(x)
#endif
