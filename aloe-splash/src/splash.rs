/*!
  | In accordance with the terms of the Aloe
  | 6 End-Use License Agreement, the Aloe Code in
  | SECTION A cannot be removed, changed or
  | otherwise rendered ineffective unless you have
  | a Aloe Indie or Pro license, or are using Aloe
  | under the GPL v3 license.
  |
  | End User License Agreement:
  | www.aloe.com/aloe-6-licence
  */

crate::ix!();

//-------------------------------------------[.cpp/Aloe/modules/aloe_gui_basics/misc/aloe_ALOESplashScreen.h]
//-------------------------------------------[.cpp/Aloe/modules/aloe_gui_basics/misc/aloe_ALOESplashScreen.cpp]

/* ----------------- BEGIN SECTION A  ----------------- */

#[cfg(not(ALOE_DISPLAY_SPLASH_SCREEN))]
pub const ALOE_DISPLAY_SPLASH_SCREEN:  usize = 1;

#[cfg(not(ALOE_USE_DARK_SPLASH_SCREEN))]
pub const ALOE_USE_DARK_SPLASH_SCREEN: usize = 1;

pub const milliseconds_to_display_splash: i32 = 2000;
pub const splash_screen_fade_out_time:    i32 = 2000;
pub const splash_screen_logo_width:       i32 = 123;
pub const splash_screen_logo_height:      i32 = 63;
...
pub fn get_logo_area(parent_rect: Rectangle<f32>) -> Rectangle<f32> {
...
/**
  | The standard Aloe splash screen component.
  | 
  | @tags{GUI}
  |
  */
#[no_copy]
#[leak_detector]
pub struct ALOESplashScreen<'a> {
    base:    Component<'a>,
    base2:   Timer,
    base3:   DeletedAtShutdown,
    content: Box<Drawable<'a>>,
    fader:   ComponentAnimator<'a>,
}

impl<'a> ALOESplashScreen<'a> {
    
    pub fn new(parent: &mut Component) -> Self {
    ...
    pub fn get_splash_screen_logo(&mut self) -> Box<Drawable> {
        
        todo!();
        /*
            const char* svgData = R"ALOESPLASHSCREEN(
    ...
    pub fn mouse_up(&mut self, _0: &MouseEvent)  {
        ...
        todo!();
        /*
            Url aloeWebsite ("https://aloe.com");
        aloeWebsite.launchInDefaultBrowser();
        */
    }
