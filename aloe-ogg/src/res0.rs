/*!
  function: residue backend 0, 1 and
  2 implementation
  */

crate::ix!();

//-------------------------------------------[.cpp/Aloe/modules/aloe_audio_formats/codecs/oggvorbis/libvorbis-1.3.7/lib/res0.c]

/**
  | Slow, slow, slow, simpleminded and
  | did I mention it was slow? The encode/decode
  | loops are coded for clarity and performance
  | is not yet even a nagging little idea
  | lurking in the shadows. Oh and BTW, it's
  | slow.
  |
  */
pub struct VorbisLookResidue0 {
    info:          *mut VorbisInfoResidue0,
    parts:         i32,
    stages:        i32,
    fullbooks:     *mut Codebook,
    phrasebook:    *mut Codebook,
    partbooks:     *mut *mut *mut Codebook,
    partvals:      i32,
    decodemap:     *mut *mut i32,
    postbits:      i64,
    phrasebits:    i64,
    frames:        i64,

    #[cfg(any(TRAIN_RES,TRAIN_RESAUX))] train_seq:     i32,
    #[cfg(any(TRAIN_RES,TRAIN_RESAUX))] training_data: [[*mut i64; 8]; 64],
    #[cfg(any(TRAIN_RES,TRAIN_RESAUX))] training_max:  [[f32; 8]; 64],
    #[cfg(any(TRAIN_RES,TRAIN_RESAUX))] training_min:  [[f32; 8]; 64],
    #[cfg(any(TRAIN_RES,TRAIN_RESAUX))] tmin:          f32,
    #[cfg(any(TRAIN_RES,TRAIN_RESAUX))] tmax:          f32,
    #[cfg(any(TRAIN_RES,TRAIN_RESAUX))] submap:        i32,
}

pub fn res0_free_info(i: *mut VorbisInfoResidue)  {
...
pub fn res0_free_look(i: *mut VorbisLookResidue)  {
...
pub fn res0_unpack(
        vi:  *mut VorbisInfo,
        opb: *mut OggPackBuffer) -> *mut VorbisInfoResidue {
...
pub fn res0_look(
        ...
        vr: *mut VorbisInfoResidue) -> *mut VorbisLookResidue {
...
/**
  | break an abstraction and copy some code
  | for performance purposes
  |
  */
pub fn local_book_besterror(
        book: *mut Codebook,
        a:    *mut i32) -> i32 {
...
/**
  | designed for stereo or other modes where
  | the partition size is an integer multiple
  | of the number of channels encoded in
  | the current submap
  |
  */
pub fn _2class(
...
pub fn zero_one_forward(
        opb:      *mut OggPackBuffer,
        vl:       *mut VorbisLookResidue,
        in_:      *mut *mut i32,
        ch:       i32,
        partword: *mut *mut i64,

        #[cfg(not(TRAIN_RES))]
        encode:   fn(
                _0: *mut OggPackBuffer,
                _1: *mut i32,
                _2: i32,
                _3: *mut Codebook
        ) -> i32,
...
/**
  | a truncated packet here just means 'stop
  | working'; it's not an error
  |
  */
pub fn _01inverse(
        vb:         *mut VorbisBlock,
...
/**
  | res2 is slightly more different; all
  | the channels are interleaved into a
  | single vector and encoded.
  |
  */
pub fn res2_forward(
        opb:      *mut OggPackBuffer,
...
/**
  | duplicate code here as speed is somewhat
  | more important
  |
  */
pub fn res2_inverse(
