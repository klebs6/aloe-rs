/*!
  ...
  |
  | A 'chained bitstream' is a Vorbis bitstream
  | that contains more than one logical
  | bitstream arranged end to end (the only
  | form of Ogg multiplexing allowed in
  | a Vorbis bitstream; grouping [parallel
  | multiplexing] is not allowed in Vorbis)
  | 
  | A Vorbis file can be played beginning
  | to end (streamed) without worrying
  | ahead of time about chaining (see decoder_example.c).
  | If we have the whole file, however, and
  | want random access (seeking/scrubbing)
  | or desire to know the total length/time
  | of a file, we need to account for the possibility
  | of chaining.
  | 
  | We can handle things a number of ways;
  | we can determine the entire bitstream
  | structure right off the bat, or find
  | pieces on demand.
  | 
  | This example determines and caches
  | structure for the entire bitstream,
  | but builds a virtual decoder on the fly
  | when moving between links in the chain.
  | 
  | There are also different ways to implement
  | seeking. Enough information exists
  | in an Ogg bitstream to seek to sample-granularity
  | positions in the output. Or, one can
  | seek by picking some portion of the stream
  | roughly in the desired area if we only
  | want coarse navigation through the
  | stream.
  | 
  | Many, many internal helpers. The intention
  | is not to be confusing; rampant duplication
  | and monolithic function implementation
  | would be harder to understand anyway.
  | The high level functions are last. Begin
...
//-------------------------------------------[.cpp/Aloe/modules/aloe_audio_formats/codecs/oggvorbis/libvorbis-1.3.7/lib/vorbisfile.c]

/*
  | read a little more data from the file/pipe
  | into the ogg_sync framer
  |
  */
...
/**
  | a smaller read size is needed for low-rate
  | streaming.
  |
  */
pub const READSIZE: usize = 2048;

pub fn get_data(vf: *mut OggVorbis_File) -> i64 {
...
/**
  | save a tiny smidge of verbosity to make
...
/**
  | The read/seek functions track absolute
  | position within the stream
  | 
  | from the head of the stream, get the next
  | page. boundary specifies if the function
  | is allowed to fetch more data from the
  | stream (and how much) or only use internally
  | buffered data.
  | 
  | boundary:
  | 
  ...
  | return:
  | 
  | <0) did not find a page (OV_FALSE, OV_EOF, OV_EREAD)
...
/**
  | find the latest page beginning before
  | the passed in position. Much dirtier
  | than the above as Ogg doesn't have any
  | backward search linkage. no 'readp'
  | as it will certainly have to read.
  | 
...
/**
  ...
  |
  */
pub fn lookup_serialno(
        s:             i64,
        serialno_list: *mut i64,
        n:             i32) -> i32 {
...
/**
  | performs the same search as _get_prev_page,
  | but prefers pages of the specified serial
  | number. If a page of the specified serialno
  | is spotted during the seek-back-and-read-forward,
  | it will return the info of last page of
  | the matching serial number instead
  | of the very last page. If no page of the
  | specified serialno is seen, it will
  | return the info of last page and alter
  | *serialno.
  |
  */
pub fn get_prev_page_serial(
        vf:          *mut OggVorbis_File,
        begin:       i64,
        serial_list: *mut i64,
        serial_n:    i32,
...
/**
  | uses the local ogg_stream storage in
  | vf; this is important for non-streaming
  | input sources
  |
  */
...
/**
  ...
  | get initial PCM offset of next page.
  | Consumes the page in the process without
  | decoding audio, however this is only
  | called during stream parsing upon seekable
  | open.
  |
  */
pub fn initial_pcmoffset(
        vf: *mut OggVorbis_File,
        vi: *mut VorbisInfo) -> i64 {
...
/**
  | finds each bitstream link one at a time
  | using a bisection search (has to begin
  | by knowing the offset of the lb's initial
  | page).
  | 
  | Recurses for each link so it can alloc
  | the link storage after finding them
  | all, then unroll and fill the cache at
  | the same time
  |
  */
pub fn bisect_forward_serialno(
        vf:             *mut OggVorbis_File,
        begin:          i64,
        searched:       i64,
        end:            i64,
...
pub fn make_decode_ready(vf: *mut OggVorbis_File) -> i32 {
...
pub fn open_seekable2(vf: *mut OggVorbis_File) -> i32 {
...
/**
  | clear out the current logical bitstream
  | decoder
  |
  */
pub fn decode_clear(vf: *mut OggVorbis_File)  {
...
/**
  | fetch and process a packet. Handles
  | the case where we're at a bitstream boundary
  | and dumps the decoding machine. If the
  | decoding machine is unloaded, it loads
  | it. It also keeps pcm_offset up to date
  | (seek and read both use this. seek uses
  | a special hack with readp).
  | 
  | return:
  | 
  | <0) error, OV_HOLE (lost packet) or
  | OV_EOF
  | 
  ...
  |
  */
pub fn fetch_and_process_packet(
        vf:    *mut OggVorbis_File,
        op_in: *mut OggPacket,
        readp: i32,
        spanp: i32) -> i32 {
...
/**
  | if, eg, 64 bit stdio is configured by
  | default, this will build with fseek64
  |
  */
pub fn fseek64_wrap(
        f:      *mut libc::FILE,
...
pub fn ov_open2(vf: *mut OggVorbis_File) -> i32 {
...
/**
  ...
  |
  */
pub fn ov_clear(vf: *mut OggVorbis_File) -> i32 {
...
/**
  | inspects the OggVorbis file and finds/documents
  | all the logical bitstreams contained
  | in it. Tries to be tolerant of logical
...
/**
  | cheap hack for game usage where downsampling
  | is desirable; there's no need for SRC
  | as we can just do it cheaply in libvorbis.
  |
  */
pub fn ov_halfrate(
        vf:   *mut OggVorbis_File,
...
pub fn ov_halfrate_p(vf: *mut OggVorbis_File) -> i32 {
...
/**
  | Only partially open the vorbis file;
  | test for Vorbisness, and load the headers
  | for the first chain. Do not seek (although
  | test for seekability). Use ov_test_open
  | to finish opening the file, else ov_clear
  | to close/free it. Same return codes
  | as open.
  | 
  | -----------
  | @note
  | 
  | vorbisfile does _not_ take ownership
  | of the file if the call fails; the calling
  | applicaiton is responsible for closing
...
pub fn ov_test_open(vf: *mut OggVorbis_File) -> i32 {
...
/**
  | How many logical bitstreams in this
  | physical bitstream?
  |
  */
pub fn ov_streams(vf: *mut OggVorbis_File) -> i64 {
...
/**
  ...
  |
  */
pub fn ov_seekable(vf: *mut OggVorbis_File) -> i64 {
...
/**
  | returns the bitrate for a given logical
  | bitstream or the entire physical bitstream.
  | If the file is open for random access,
  | it will find the *actual* average bitrate.
  | If the file is streaming, it returns
  | the nominal bitrate (if set) else the
  | average of the upper/lower bounds (if
  | set) else -1 (unset).
  | 
  | If you want the actual bitrate field
  | settings, get them from the vorbis_info
  | structs
  |
  */
pub fn ov_bitrate(
...
/**
  | returns the actual bitrate since last
  | call. returns -1 if no additional data
  | to offer since last call (or at beginning
  | of stream),
  | 
  | EINVAL if stream is only partially open
  |
  */
pub fn ov_bitrate_instant(vf: *mut OggVorbis_File) -> i64 {
...
/**
  | returns: total raw (compressed) length
  | of content if i==-1 raw (compressed)
  | length of that logical bitstream for
  | i==0 to n
  | 
  | OV_EINVAL if the stream is not seekable
  | (we can't know the length) or if stream
...
/**
  | returns: total PCM length (samples)
  | of content if i==-1 PCM length (samples)
  | of that logical bitstream for i==0 to
  | n
  | 
  | OV_EINVAL if the stream is not seekable
  | (we can't know the length) or only partially
  | open
  |
  */
pub fn ov_pcm_total(
...
/**
  ...
  | i==-1 seconds in that logical bitstream
  | for i==0 to n
  | 
  | OV_EINVAL if the stream is not seekable
  | (we can't know the length) or only partially
...
/**
  | seek to an offset relative to the *compressed*
  | data. This also scans packets to update
  | the PCM cursor. It will cross a logical
  | bitstream boundary, but only if it can't
  | get any packets out of the tail of the
  | bitstream we seek to (so no surprises).
  | 
  | returns zero on success, nonzero on
  | failure
  |
  */
pub fn ov_raw_seek(
...
/**
  | Page granularity seek (faster than
  | sample granularity because we don't
  | do the last bit of decode to find a specific
  | sample).
  | 
  | Seek to the last [granule marked] page
  | preceding the specified pos location,
  | such that decoding past the returned
  | point will quickly arrive at the requested
  | position.
  |
  */
pub fn ov_pcm_seek_page(
        vf:  *mut OggVorbis_File,
        pos: i64) -> i32 {
...
/**
  | seek to a sample offset relative to the
  | decompressed pcm stream returns zero
...
/**
  | seek to a playback time relative to the
  | decompressed pcm stream returns zero
  | on success, nonzero on failure
  |
  */
...
/**
  | tell the current stream offset cursor.
  | Note that seek followed by tell will
  | likely not give the set offset due to
  | caching
  |
  */
pub fn ov_raw_tell(vf: *mut OggVorbis_File) -> i64 {
...
/**
  | return PCM offset (sample) of next PCM
  | sample to be read
  |
  */
pub fn ov_pcm_tell(vf: *mut OggVorbis_File) -> i64 {
...
/**
  | return time offset (seconds) of next
  | PCM sample to be read
  |
  */
...
/**
  ...
  | 
  -------------------
  | -1) return the vorbis_info struct for
  | the bitstream section currently being
  | decoded
  | 
  -------------------
  | 0-n) to request information for a specific
  | bitstream section
  | 
  -------------------
  | In the case of a non-seekable bitstream,
  | any call returns the current bitstream.
  | NULL in the case that the machine is not
  | initialized
  |
  */
pub fn ov_info(
        vf:   *mut OggVorbis_File,
        link: i32) -> *mut VorbisInfo {
...
/**
  | grr, strong typing, grr, no templates/inheritence,
  | grr
  |
  */
pub fn ov_comment(
        vf:   *mut OggVorbis_File,
        link: i32) -> *mut VorbisComment {
...
/**
  | up to this point, everything could more
  | or less hide the multiple logical bitstream
  | nature of chaining from the toplevel
  | application if the toplevel application
  | didn't particularly care. However,
  | at the point that we actually read audio
  | back, the multiple-section nature
  | must surface: Multiple bitstream sections
  | do not necessarily have to have the same
  | number of channels or sampling rate.
  | 
  | ov_read returns the sequential logical
  | bitstream number currently being decoded
  | along with the PCM data in order that
  | the toplevel application can take action
  | on channel/sample rate changes. This
  | number will be incremented even for
  | streamed (non-seekable) streams (for
  | seekable streams, it represents the
  | actual logical bitstream index within
  | the physical bitstream. Note that the
  | accessor functions above are aware
  | of this dichotomy).
  | 
  | ov_read_filter is exactly the same
  | as ov_read except that it processes
  | the decoded audio data through a filter
  | before packing it into the requested
  | format. This gives greater accuracy
  | than applying a filter after the audio
  | has been converted into integral PCM.
  | 
  -------------------
  | input values:
  | 
  -------------------
  | buffer) a buffer to hold packed PCM data
  | for return
  | 
  -------------------
  | length) the byte length requested to
  | be placed into buffer
  | 
  -------------------
  | bigendianp) should the data be packed
  | LSB first (0) or
  | MSB first (1)
  | 
  -------------------
  | word) word size for output. currently
  | 1 (byte) or 2 (16 bit short)
  | 
  -------------------
  | return values:
  | 
  -------------------
  | <0) error/hole in data (OV_HOLE), partial
  | open (OV_EINVAL)
  | 
  -------------------
  | 0) EOF
  ...
  | The below works on a packet-by-packet
  | basis, so the return length is not related
  | to the 'length' passed in, just guaranteed
  | to fit.
  | 
  -------------------
  | section) set to the logical bitstream
  | number
  |
  */
pub fn ov_read_filter(
        ...
        filter:       fn(
                ...
                channels:     i64,
                samples:      i64,
                filter_param: *mut c_void
        ) -> c_void,
        filter_param: *mut c_void) -> i64 {
...
/**
  | input values: pcm_channels) a float
  | vector per channel of output length)
  | the sample length being read by the app
  | 
  | return values:
  | 
  ------------------------
  ...
  | 
  ------------------------
  | n) number of samples of PCM actually
  | returned. The below works on a packet-by-packet
  | basis, so the return length is not related
  | to the 'length' passed in, just guaranteed
  | to fit.
  | 
...
/**
  ...
  |
  */
pub fn ov_initset(vf: *mut OggVorbis_File) -> i32 {
...
/**
  ...
  | a primed buffer; if we're crosslapping
  | at a stream section boundary, this also
  | makes sure we're sanity checking against
  | the right stream information
  |
  */
pub fn ov_initprime(vf: *mut OggVorbis_File) -> i32 {
...
/**
  | grab enough data for lapping from vf;
  | this may be in the form of unreturned,
  | already-decoded pcm, remaining PCM
  | we will need to decode, or synthetic
  | postextrapolation from last packets.
  |
  */
pub fn ov_getlap(
        vf:      *mut OggVorbis_File,
...
/**
  | this sets up crosslapping of a sample
  | by using trailing data from sample 1
  | and lapping it into the windowing buffer
  | of sample 2
  |
  */
pub fn ov_crosslap(
        vf1: *mut OggVorbis_File,
        vf2: *mut OggVorbis_File) -> i32 {
...
pub fn ov_64_seek_lap(
        vf:        *mut OggVorbis_File,
        pos:       i64,
        localseek: fn(_0: *mut OggVorbis_File, _1: i64) -> i32) -> i32 {
...
pub fn ov_d_seek_lap(
        vf:        *mut OggVorbis_File,
        pos:       f64,
        localseek: fn(_0: *mut OggVorbis_File, _1: f64) -> i32) -> i32 {
