/*!
 function: PCM data vector blocking, windowing and
 dis/reassembly

 Handle windowing, overlap-add, etc of the PCM
 vectors.  This is made more amusing by Vorbis'
 current two allowed block sizes.
*/

crate::ix!();

//-------------------------------------------[.cpp/Aloe/modules/aloe_audio_formats/codecs/oggvorbis/libvorbis-1.3.7/lib/block.c]
...
/** 
  ...
  */
pub fn vorbis_block_ripcord(vb: *mut VorbisBlock)  {
...
pub fn vorbis_block_clear(vb: *mut VorbisBlock) -> i32 {
...
/**
  | Analysis side code, but directly related
  | to blocking. Thus it's here and not in
  | analysis.c (which is for analysis transforms
  | only). The init is here because some
  | of it is shared
  |
  */
pub fn vds_shared_init(
        v:    *mut VorbisDspState,
        vi:   *mut VorbisInfo,
        encp: i32) -> i32 {
...
/** 
  arbitrary settings and spec-mandated numbers get
  filled in here 
  */
...
pub fn vorbis_dsp_clear(v: *mut VorbisDspState)  {
...
pub fn preextrapolate_helper(v: *mut VorbisDspState)  {
...
/**
  | do the deltas, envelope shaping, pre-echo
  | and determine the size of the next block
  | on which to continue analysis
  |
  */
...
pub fn vorbis_synthesis_restart(v: *mut VorbisDspState) -> i32 {
...
/**
  | Unlike in analysis, the window is only
  | partially applied for each block. The
  | time domain envelope is not yet handled
  | at the point of calling (as it relies
...
/** 
  pcm==NULL indicates we just want the pending
  samples, no more 
  */
pub fn vorbis_synthesis_pcmout(
        v:   *mut VorbisDspState,
        pcm: *mut *mut *mut f32) -> i32 {
...
/**
  | intended for use with a specific vorbisfile
  | feature; we want access to the [usually
  | synthetic/postextrapolated] buffer
  | and lapping at the end of a decode cycle,
  | specifically, a half-short-block
  | worth. This funtion works like pcmout
  | above, except it will also expose this
