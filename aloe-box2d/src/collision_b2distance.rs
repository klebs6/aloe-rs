...
//-------------------------------------------[.cpp/Aloe/modules/aloe_box2d/box2d/Collision/b2Distance.h]
...
impl Default for b2DistanceProxy {
...
/**
  ...
  |
  | You have to option to use the shape radii in
  | the computation. Even
  */
...
/**
   GJK using Voronoi regions (Christer Ericson)
...
//-------------------------------------------[.cpp/Aloe/modules/aloe_box2d/box2d/Collision/b2Distance.cpp]
impl b2DistanceProxy {
    ...
    /**
      ...
      |
      */
    #[inline] pub fn get_vertex_count(&self) -> i32 {
    ...
    /**
      | Initialize the proxy using the given
      | shape. The shape must remain in scope
...
impl b2Simplex {
    ...
    pub fn write_cache(&self, cache: *mut b2SimplexCache)  {
    ...
    pub fn get_search_direction(&self) -> b2Vec2 {
    ...
    pub fn get_closest_point(&self) -> b2Vec2 {
    ...
    pub fn get_witness_points(&self, 
        pa: *mut b2Vec2,
        pb: *mut b2Vec2)  {
    ...
    /**
      | Solve a line segment using barycentric
      | coordinates.
      |
      | p = a1 * w1 + a2 * w2
      | a1 + a2 = 1
      |
      | The vector from the origin to the closest
      | point on the line is perpendicular to the
      | line.
      |
      | e12 = w2 - w1
      | dot(p, e) = 0
...
/**
  | Compute the closest points between two
  | shapes. Supports any combination of:
  |
  | b2CircleShape, b2PolygonShape,
  | b2EdgeShape. The simplex cache is
  | input/output.
  |
  | On the first call set b2SimplexCache.count to
  | zero.
  */
pub fn b2distance(
