/*!
  | This file contains a bunch of miscellaneous
  | utilities that are used by the various
  | demos.
  | 
  | This signing key is used to sign the android
  | version of the in-app purchase sample
  | app so that authenticated requests
  | to the play store API can be tested.
  | 
  | This key has only been used to sign the
  | in-app purchase sample app and therefore
  | cannot be used for any other apps. It’s
  | impossible to use to sign any existing
  | apps on the app store.
  | 
  | ! Do not use this key for your own production
  | apps
  |
  */

//-------------------------------------------[.cpp/Aloe/examples/DemoRunner/Builds/Android/app/src/main/assets/Signing/README.txt]
//-------------------------------------------[.cpp/Aloe/examples/DemoRunner/Builds/Android/app/src/main/assets/DemoUtilities.h]

crate::ix!();

#[cfg(not(PIP_DEMO_UTILITIES_INCLUDED))]
pub const PIP_DEMO_UTILITIES_INCLUDED: usize = 1;

#[inline] pub fn get_random_colour(brightness: f32) -> Colour {
...
#[inline] pub fn get_random_bright_colour() -> Colour {
    
    todo!();
    /*
        return getRandomColour (0.8f);
    */
}

#[inline] pub fn get_random_dark_colour() -> Colour {
    
    todo!();
    /*
        return getRandomColour (0.3f);
    */
}

#[inline] pub fn get_ui_colour_if_available(
    ui_colour: LookAndFeelV4ColourSchemeUIColour,
    fallback:  Option<Colour>

) -> Colour {

    let fallback: Colour = fallback.unwrap_or(Colour::new_from_argb(0xff4d4d4d));
...
#[inline] pub fn get_examples_directory() -> File {
    
    todo!();
    /*
        #ifdef PIP_ALOE_EXAMPLES_DIRECTORY
        MemoryOutputStream mo;

        auto success = Base64::convertFromBase64 (mo, ALOE_STRINGIFY (PIP_ALOE_EXAMPLES_DIRECTORY));
        ignoreUnused (success);
        jassert (success);

        return mo.toString();
       #elif defined PIP_ALOE_EXAMPLES_DIRECTORY_STRING
        return File { CharPointer_UTF8 { PIP_ALOE_EXAMPLES_DIRECTORY_STRING } };
       #else
...
#[inline] pub fn create_asset_input_stream(resource_path: *const u8) -> Box<dyn Read> {
...
#[inline] pub fn get_image_from_assets(asset_name: *const u8) -> Image {
    
    todo!();
    /*
        auto hashCode = (String (assetName) + "@aloe_demo_assets").hashCode64();
        auto img = ImageCache::getFromHashCode (hashCode);

        if (img.isNull())
        {
            std::unique_ptr<InputStream> aloeIconStream (createAssetInputStream (assetName));

            if (aloeIconStream == nullptr)
                return {};

            img = ImageFileFormat::loadFrom (*aloeIconStream);
...
#[inline] pub fn load_entire_asset_into_string(asset_name: *const u8) -> String {
...
#[inline] pub fn get_aloe_logo_path() -> PathBuf {
...
#[inline] pub fn get_dark_code_editor_colour_scheme() -> CodeEditorComponent::ColourScheme {
...
#[cfg(ALOE_MODULE_AVAILABLE_aloe_gui_extra)]
#[inline] pub fn get_light_code_editor_colour_scheme() -> CodeEditorComponent::ColourScheme {
