...
//-------------------------------------------[.cpp/Aloe/modules/aloe_audio_devices/midi_io/ump/aloe_UMPTests.cpp]

#[cfg(ALOE_UNIT_TESTS)]
pub mod universal_midi_packets {
    use super::*;

    pub struct UniversalMidiPacketTests {
        base: UnitTest,
    }

    impl Default for UniversalMidiPacketTests {
    ...
    #[cfg(all(target_os="windows",not(ALOE_MINGW)))]
    macro_rules! aloe_checked_iterator {
        ($msg:ident, $size:ident) => {
            /*
    ...
    #[cfg(not(all(target_os="windows",not(ALOE_MINGW))))]
    macro_rules! aloe_checked_iterator {
        ($msg:ident, $size:ident) => {
            /*
                    (msg)
    ...
    impl UniversalMidiPacketTests {
        ...
        pub fn convert_midi_2to_midi1(midi2: &Packets) -> Packets {
        ...
        pub fn convert_midi_1to_midi2(midi1: &Packets) -> Packets {
        ...
        pub fn check_bytestream_conversion(&mut self, 
            actual:   &Packets,
            expected: &Packets)  {
        ...
        pub fn check_midi_2to_midi_1conversion(&mut self, 
            midi2:    &Packets,
            expected: &Packets)  {
        ...
        pub fn check_midi_1to_midi_2conversion(&mut self, 
            midi1:    &Packets,
            expected: &Packets)  {
        ...
        pub fn create_random_sys_ex(&mut self, 
            random:       &mut Random,
            sys_ex_bytes: usize) -> MidiMessage {
        ...
        pub fn create_random_utilityump(&mut self, random: &mut Random) -> PacketX1 {
        ...
        pub fn create_random_realtimeump(&mut self, random: &mut Random) -> PacketX1 {
        ...
        pub fn for_each_non_sys_ex_test_message<Fn>(&mut self, 
            random: &mut Random,
            fn_:    Fn)  {
            ...
            /*
                return a.getRawDataSize() == b.getRawDataSize()
                       && std::equal (a.getRawData(), a.getRawData() + a.getRawDataSize(),
                                      ALOE_CHECKED_ITERATOR (b.getRawData(), b.getRawDataSize()));
            */
        }
